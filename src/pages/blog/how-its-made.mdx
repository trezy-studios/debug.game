...
<BlogArticlePage
	publishedAt={'2023-05-17T05:30:00.000Z'}
	subtitle={'The tech behind Debug.'}
	title={'How It\'s Made'}>
	...
	* **Give users fine-grained control over the game's settings**, which is often challenging to implement in JavaScript due to the sandboxed environment.
	...
	Most games are using engines like Unity, Unreal, or Godot to build for the desktop (or aging custom engines at a lot of AAA studios, but that's a whole different article). There are a couple of JavaScript engines that can compile for multiple platforms — such as [Construct 3](https://www.construct.net) and [Impact.js](https://impactjs.com) — but I wanted the freedom to choose what technologies I used to build every aspect of the game.
	...
	We can get away with ditching a lot of typical game engine needs since we only have 1 character, and the whole game is 2D. Instead of using something off-the-shelf, I decided to build everything from scratch. This was ~~probably~~ definitely a bad choice, but I've been enjoying myself anyway. 😅
	...
	`ngraph` is probably even worth its own article. I even used it to implement really effective UI navigation for gamepads. 👀
	...
	There you have it - a behind–the–scenes look into the tech stack that makes Debug tick. From JavaScript as our language of choice, Electron for desktop publishing, React and Framer Motion for creating the UI, to Pixi, ngraph, and handcrafted systems for game mechanics, and finally, Statery for state management.
